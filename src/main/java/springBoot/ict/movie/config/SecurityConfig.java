	...
	public  SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
		...
		http
			...
			.authorizeHttpRequests((requests) -> requests
					.antMatchers(HttpMethod.POST, "/login", "/register", "/sign-up", "/index/save", "/admin/**", "/page_5/**", "/page_3/**", "/**", "oauth2/**", "/page_1/**").permitAll()   // 인증이 필요하지 않은 유일한 엔드포인트이며, 리액트의 url과 일치시켜야 한다.
					.antMatchers(HttpMethod.GET, "/admin/**", "/page_5/**", "/page_3/**","/**", "oauth2/**", "/page_1/**").permitAll()
					.antMatchers(HttpMethod.PUT, "/page_1/**").permitAll()

...
import style from "../../../styles/page_5/movieDetail4.module.css";
  ...
  return (
    <ul>
      {[...Array(maxStars).keys()].map((index) => (
        <li
          key={index + 1}
          className={`star_box${index + 1}`}
          onClick={() => onStarClick(index + 1)}
        >
          <img
            className={`star_on${index + 1} ${
              selectedStars >= index + 1 ? `${style.star_on}${index + 1}` : ""
            }`}
...
function MovieDetail4() {
  ...
  const { next_id } = useParams(); // useParams 훅을 사용하여 URL의 id 값을 가져옴
  ...
  // 관람평
  const reviewList = (next_id) => {
    console.log("reviewList start");
    ApiService.reviewList(next_id)
      .then((res) => {
    ...
    <>
      <div className={`detail_movie_wrap ${style.detail_movie_wrap}`}>
        <div className={`detail_info ${style.detail_info}`}>
          <div className={`detail_img_box ${style.detail_img_box}`}>
            <img
              src={`${process.env.PUBLIC_URL}/page_5/${detailInfo.next_image}`}
              alt="파묘상세"
              style={{ width: "250px" }}
            />
          </div>
          ...
          <div className={`detail_text ${style.detail_text}`}>
            <div className={`detail_title ${style.detail_title}`}>
              <label for="pp_name">{detailInfo.next_title}</label>
            </div>

            <div className={`detail_con ${style.detail_con}`}>
              <div className={`detail_con1 ${style.detail_con1}`}>
                <label for="de_date">
                  {new Date(detailInfo.next_date).toLocaleDateString()} 개봉
                </label>
                <label for="de_time">| {detailInfo.next_time}분 |</label>
                <label for="de_age">
                  {" "}
                  <span style={{ color: "orange" }}>{detailInfo.next_age}</span>
                </label>
                <label for="de_num">| {detailInfo.next_visitor}</label>
              </div>
            </div>

            <div className={`btn_wrap1 ${style.btn_wrap1}`}>
              <button
                className={`detail_btn1 ${style.detail_btn1}`}
                id="10"
                onClick={() => handleTrailerClick(detailInfo.next_trailer1)}
              >
                &#9658;예고편 재생{" "}
              </button>
            ...
            <div className={`detail_box1 ${style.detail_box1}`}>
              {selectedValue === 1 && (
                <div>
                  ...
                  <p>
                    <span style={{ color: "gray" }}>.감독</span>{" "}
                    {detailInfo.next_pd}
                  </p>
                  <p>
                    <span style={{ color: "gray" }}>.출연</span>{" "}
                    {detailInfo.next_cast}
                  </p>
                </div>
              )}
                ...
                <div>
                  ...
                  {!cookies.idName && (
                    <div className={`plz_login ${style.plz_login}`}>
                      ...
                      <Link
                        to="/login"
                        className={`plz_login_button ${style.plz_login_button}`}
                      >
                        <div>로그인</div>
                      </Link>
                    </div>
                  ...
                  {cookies.idName && (
                    <div className={`star_info ${style.star_info}`}>
                      ...
                      <div className={`star_img_box ${style.star_img_box}`}>
                        <img
                          src={`https://www.lottecinema.co.kr/NLCHS/Content/images/icon/icon_reviewcharacterbig_${selectedStars}.svg`}
                          alt={`star${selectedStars}`}
                        />
                      </div>
                    </div>
                  )}
                ...
                {getReviewlists.map((review, index) => {
                  ...
                  return (
                    ...
                    >
                      {/* 이미지 파일 경로를 동적으로 설정 */}
                      <img
                        src={`https://www.lottecinema.co.kr/NLewcharacter_0${
                          6 - review.review_star
                        }.jpg`}
                        alt="image_by_rate"
                        style={{
                          marginRight: "10px",
                      ...
                      <div
                        className={`movie_review_content ${style.movie_review_content}`}
                      >
                        <p>작성자</p>
                      ...
                      <div style={{ marginLeft: "auto" }}>
                        <img
                          src="https://www.lottecinema.co.kr/NLCHS/Content/images/icon/ic_review_good.png"
                          alt="image_by_rate"
                          style={{
                            marginRight: "5px",
                            width: "20px",
        ...
        >
          <img
            src={`${process.env.PUBLIC_URL}/page_5_1/${detailInfo.next_image2}`}
            alt="트레일러1"
          />
        </button>
        <button
          className={`detail_trailer1 ${style.detail_trailer2}`}
          onClick={() => handleTrailerClick(detailInfo.next_trailer2)}
        >
          <img
            src={`${process.env.PUBLIC_URL}/page_5_1/${detailInfo.next_image3}`}
            alt="트레일러2"
          />
        </button>
      </div>
      ...
      <div className={`detail_last ${style.detail_last}`}>
        <ul>
          <li>
            <a href="#">
              <img src={BobMarley_OneLove} alt="detail.img" />
            </a>
          </li>
        </ul>
      </div>

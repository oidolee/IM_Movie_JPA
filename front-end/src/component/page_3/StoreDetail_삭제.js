...
// detail
fetchStoreByID = (itemCode) => {
  ApiService.fetchStoreByID(itemCode) // 백엔드 API에서 해당 itemCode에 대한 상세 정보를 가져옵니다.
  .then(res => {
      // 가져온 상세 정보를 state에 저장하거나 다른 작업을 수행합니다.
      this.setState({
          detail: res.data // 가져온 상세 정보를 state에 저장합니다.
      });
  })
  .catch(err => {
      console.log('getStoreDetail() Error:', err);
  });
}
      ...
      <div className={`store_detail ${style.store_detail}`}>
        <div className={`main_img ${style.main_img}`}>
          ...
          <div style={{ width: "450px" }}>
            ...
            >
              <SwiperSlide>
                <img src={package1} style={{ width: "150px", left: "0" }} />
              </SwiperSlide>
              <SwiperSlide>
                <img src={package1} style={{ width: "150px", left: "0" }} />
              </SwiperSlide>
